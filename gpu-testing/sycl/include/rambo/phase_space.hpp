...
struct RamboAlgorithm {
    ...
    static auto newtonSolve(double& x, FuncType f, DerivType df,
        ...
        for (int iter = 0; iter <= maxIter; ++iter) {
            double fVal = f(x);
            if (sycl::fabs(fVal) <= tol) return iter;
            double dfVal = df(x);
...
public:
    auto generate(double cmEnergy, const double r[4 * nParticles], 
        ...
        double x = sycl::sqrt(1.0 - totalMassSq / cmEnergySq);
        for (int i = 0; i < nParticles; ++i) {
            momSq[i] = p[i][0] * p[i][0];
        }
        ...
        newtonSolve(x,
            ...
            [&](double xVal) {
                ...
                double x2 = xVal * xVal;
                for (int i = 0; i < nParticles; ++i) {
                    double e = sycl::sqrt(massSq[i] + x2 * momSq[i]);
                    df += xVal * momSq[i] / e;
                }
                return df;
            },
...
public:
    ...
    auto generate(double cmEnergy, const double r[3 * nParticles - 4],
        ...
        if constexpr (nParticles == 2) {
            ...
            p[0][0] = q;
            p[0][1] = q * sinTheta * sycl::cos(phi);
            p[0][2] = q * sinTheta * sycl::sin(phi);
            p[0][3] = q * cosTheta;
            ...
            for (int i = 1; i < nParticles; ++i) {
                if constexpr (nParticles == 3) {
                    MCurr = (i == 1) ? (1.0 - sycl::sqrt(1.0 - r[0])) * MPrev : 0.0;
                }
                else {
                    if (i < nParticles - 1) {
                ...
                sinTheta = sycl::sqrt(1.0 - cosTheta * cosTheta);
                ...
                QCurr[0] = sycl::sqrt(q * q + MCurr * MCurr);
                QCurr[1] = -p[i - 1][1];
                QCurr[2] = -p[i - 1][2];
                QCurr[3] = -p[i - 1][3];
