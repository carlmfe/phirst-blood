#pragma once
#ifndef PHIRST_RANDOM_HPP
#define PHIRST_RANDOM_HPP
 ...
 */
PHIRST_INLINE auto xorshift64(uint64_t& state) -> uint64_t {
    uint64_t x = state;
    x ^= x << 13;
    x ^= x >> 7;
    x ^= x << 17;
 ...
 */
PHIRST_INLINE auto uniformRandom(uint64_t& state) -> double {
 ...
 */
PHIRST_INLINE auto uniformRandom(uint64_t& state, double a, double b) -> double {
 ...
 */
PHIRST_INLINE auto uniformRandomInt(uint64_t& state, int64_t n) -> int64_t {
 ...
 */
PHIRST_INLINE auto initRngState(uint64_t seed, int64_t threadIdx) -> uint64_t {
